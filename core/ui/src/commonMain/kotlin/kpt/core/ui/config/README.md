# `core/ui/config` — the fork-extension seam
...
App-shell DEFAULTS a fork rebrands — scaffold behaviour, bottom-bar configuration,
navigation transitions, empty/error presentation defaults. The primitives stay in their own
packages; their tunable defaults belong here.
...
`core/ui/module-packages.yaml` — both already in place.

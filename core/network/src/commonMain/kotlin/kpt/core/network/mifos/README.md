# `mifos` — access point package

SCAFFOLDED by `./gradlew syncForkConfig` from the `mifos` access point in
...
- `api/` — the Ktorfit interface for this endpoint. Annotate it
  `@ApiBinding("mifos")` and its Koin binding is GENERATED into
  `di/GeneratedApiBindings.kt`; there is no wiring step.
- `dto/` — the wire types this endpoint returns.

`type: rest` — the binding is `restApi("mifos") { it.create${simple}Api() }`, so Ktorfit

# `core/datastore/config` — the fork-extension seam
...
Preference DEFAULTS and key vocabulary — default theme, locale, sync interval, opt-in
flags. `prefs/` holds the stores; the shipped default for each key belongs here so a fork
changes the value without forking the store.
...
`core/datastore/module-packages.yaml` — both already in place.

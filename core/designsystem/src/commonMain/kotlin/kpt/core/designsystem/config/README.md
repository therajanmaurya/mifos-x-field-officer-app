# `core/designsystem/config` — the fork-extension seam
...
Design-system CONFIGURATION that is not the palette — token scales, chart defaults,
density, motion durations. Note `theme/` is FORK-owned by design (its palette comes from the
idea-layer design pipeline and a sync must never overwrite it); this package is the
template-owned half a fork inherits and extends.
...
`core/designsystem/module-packages.yaml` — both already in place.

# `core/model/config` — the fork-extension seam
...
Domain-model DEFAULTS and shared validation constants — field length bounds, enum
fallbacks, default currency or unit. The models themselves stay in their domain packages;
only the values a fork legitimately changes belong here.
...
`core/model/module-packages.yaml` — both already in place.
